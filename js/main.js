...
function adaptive_header(w,h) {
		...
		var headerBottom=$('.header-body__bottom');
	if(w<767){
		if(!headerBottom.hasClass('done')){
			headerBottom.addClass('done').appendTo(headerMenu);
		}
	}else{
		if(headerBottom.hasClass('done')){
			headerBottom.removeClass('done').appendTo(headerMenu);
		}
	}
	...
	if(w<767){
		if(!$('.items-body__list').hasClass('done')){
			$('.items-body__list').addClass('done').appendTo(headerMenu);
		}
	}else{
		if($('.items-body__list').hasClass('done')){
			$('.items-body__list').removeClass('done').prependTo($('.header-body__seller__container__item__row__items'));
		}
	}
